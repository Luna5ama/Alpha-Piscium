...
if (modelLutBytes.size != expectedModelLutBytes) {
    failures += "PBR LUT 1 size ${modelLutBytes.size} != 64 masks * $materialCount materials * 4 ($expectedModelLutBytes)"
}
...
if (quadBytes.size % 8 != 0) failures += "quad asset size ${quadBytes.size} is not 8 bytes per quad"
val quadCount = quadBytes.size / 8
val quadTexelWidth = quadBytes.size / 4

val expectedPbrProperty = "customTexture.usam_pbrLUT1=textures/pbr_lut_1.bin TEXTURE_2D R32UI 64 $materialCount RED_INTEGER UNSIGNED_INT"
...
fun checkQuadProperty(text: String, label: String) {
    val match = Regex("(?m)^customTexture\\.usam_blockModelQuads=textures/block_model_quads\\.bin TEXTURE_1D RGBA8 (\\d+) RGBA UNSIGNED_BYTE$").find(text)
    if (match == null) failures += "$label: normalized quad texture declaration missing"
    else if (quadBytes.isNotEmpty() && match.groupValues[1].toInt() != quadTexelWidth) {
        failures += "$label: quad texture width ${match.groupValues[1]} != $quadTexelWidth"
    }
}
...
expect(textures, Regex("uniform\\s+usampler2D\\s+usam_pbrLUT1\\s*;"), "2D model LUT sampler")
expect(textures, Regex("uniform\\s+sampler1D\\s+usam_blockModelQuads\\s*;"), "normalized quad sampler")
...
expect(models, Regex("quadIndex\\s*\\*\\s*2u"), "two-texel quad stride")
...
expect(models, Regex("quadOffset\\s*=\\s*\\(modelData\\s*>>\\s*9u\\)\\s*&\\s*0xFFFFu"), "packed quad offset decode")
...
expect(builder, Regex("#define\\s+VOXEL_MATERIAL_DATA_MODIFIER\\s+(?:restrict\\s+)?buffer"), "writable packed material buffer")
val builderPacksFaceBits = Regex("faceBits\\s*<<\\s*16u").containsMatchIn(builder) ||
    Regex("bitfieldInsert\\s*\\([^)]*faceBits\\s*,\\s*16\\s*,\\s*6\\s*\\)").containsMatchIn(builder)
if (!builderPacksFaceBits) failures += "voxel tree builder does not pack open faceBits into bits 16..21"
expect(builder, Regex("voxel_materials_v4\\s*\\[[^]]+]\\s*="), "packed material writeback")
...
if (modelLutBytes.size == expectedModelLutBytes && pbrBytes.size == materialCount * 4) {
    ...
    val pbrLut = ByteBuffer.wrap(pbrBytes).order(ByteOrder.LITTLE_ENDIAN)
    ...
    for (mask in listOf(0, 63)) for (materialId in 0 until materialCount) {
...
println("Voxel trace quad contract PASS: ${allIds.size} mappings, $quadCount quads, $quadTexelWidth texels, Minecraft 26.2")
