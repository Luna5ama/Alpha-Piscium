...
expect(models, Regex("ivec2\\s+texelCoord\\s*=\\s*ivec2\\s*\\(\\s*int\\s*\\(\\s*\\(quadIndex\\s*<<\\s*1u\\)\\s*&\\s*0x7FFFu\\s*\\)\\s*,\\s*int\\s*\\(\\s*quadIndex\\s*>>\\s*14u\\s*\\)\\s*\\)"), "bitwise 2D quad texel address")
