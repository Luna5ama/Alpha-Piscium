...
val selectedModelIds = ids.filter { it >= 0 }.map { lutUInt(modelLutBytes, it) }
if (selectedModelIds.any { it == 0u }) failures += "selected non-full state has model ID zero"
...
val lutModelIds = allIds.map { lutUInt(modelLutBytes, it) }.filter { it != 0u }
