...
fun aabbOffset(modelData: UInt) = (modelData shr modelRotationBits) and 0x7FFu
fun aabbCount(modelData: UInt) = modelData shr 24
fun modelRadius(modelData: UInt) = (modelData shr 20) and 0xFu
...
if (lutModelData.any { modelRadius(it) == 0u }) failures += "model LUT contains an unbounded model radius"
fun modelBoundRadius(modelData: UInt): Double {
    val offset = aabbOffset(modelData).toInt()
    val count = aabbCount(modelData).toInt()
    var radius = 0.0
    for (index in offset until offset + count) {
        val base = index * 12
        val ox = aabbBytes[base + 4].toUByte().toDouble() / 255.0 - 0.5
        val oy = aabbBytes[base + 5].toUByte().toDouble() / 255.0 - 0.5
        val oz = aabbBytes[base + 6].toUByte().toDouble() / 255.0 - 0.5
        val hx = aabbBytes[base + 8].toUByte().toDouble() / 255.0 * 0.7071067811865475
        val hy = aabbBytes[base + 9].toUByte().toDouble() / 255.0 * 0.7071067811865475
        val hz = aabbBytes[base + 10].toUByte().toDouble() / 255.0 * 0.7071067811865475
        val centerLength = sqrt(ox * ox + oy * oy + oz * oz)
        val halfLength = sqrt(hx * hx + hy * hy + hz * hz)
        radius = maxOf(radius, centerLength + halfLength)
    }
    return radius
}
if (lutModelData.any { modelRadius(it).toDouble() * 0.1 + 1e-5 < modelBoundRadius(it) }) {
    failures += "model LUT sphere radius is not conservative for an AABB geometry"
}
...
expect(models, "uint aabbOffset = (modelData >> 9u) & 0x7FFu;", "packed AABB offset decode")
expect(models, "uint aabbCount = modelData >> 24u;", "packed AABB count decode")
expect(models, "uint modelRadius = (modelData >> 20u) & 0xFu;", "packed model radius decode")
expect(models, "for (uint i = 0u; i < aabbCount; ++i)", "AABB loop")
expect(models, "if (aabbCount == 1u)", "single-AABB fast path")
...
if (models.contains("_voxel_rotateBlockModelVector") || models.contains("_voxel_rotateBlockModelComponent")) {
    failures += "generated model code retains obsolete single-vector rotation helpers"
}
...
expect(modelFunction, "bool modelSphereHit =", "model sphere broad phase")
