...
val motionSource = File("../shaders/pass/composite/GenerateMotionVectors.comp.glsl").readText()
val superResolutionSource = File("../shaders/base/SuperResolution.glsl").readText()
val renderScaleSource = File("../shaders/util/RenderScale.glsl").readText()
val texturesSource = File("../shaders/base/Textures.glsl").readText()
val superResolutionConfigSource = File("../shaders/superresolution.v3.json").readText()
val propertiesSource = File("shaders.properties").readText()
val configsSource = File("../shaders/base/Configs.glsl").readText()
val solidGbufferSource = File("../shaders/pass/geometry/GBufferSolid.frag.glsl").readText()
...
check("#if !INTERNAL_FSR3_ACTIVE" in prepareSource && "agxInvertible_forward" in prepareSource)
check("agxInvertible_inverse" in resolveSource)
check("#if INTERNAL_FSR3_ACTIVE" in rcasSource && "agxInvertible_forward" in rcasSource && "agxInvertible_inverse" in rcasSource)
...
check("cond(\"SETTING_AA_MODE != 0 && !SR_ENABLE\")" in programsSource)
check("cond(\"SETTING_AA_MODE == 2 || SR_ENABLE\")" in programsSource)
check("cond(\"SETTING_AA_MODE == 2 && !SR_ENABLE\")" in programsSource)
...
check("#define GENERATE_MOTION_VECTORS_SUPER_RESOLUTION" in motionSource)
check("#define GENERATE_MOTION_VECTORS_FSR3" in motionSource)
check("previousUv - currentUv" in motionSource)
check("uimg_superResolutionMotionVectors" in motionSource)
check("hasTranslucentSurface" !in motionSource && "usam_gbufferTranslucentData2" !in motionSource)
check(!File("../shaders/pass/composite/FSR3MotionVectors.comp.glsl").exists())
check("#define RENDER_SCALE_ACTIVE SR_SHOULD_APPLY_SCALE" in superResolutionSource)
check("#define EXTERNAL_SR_UPSCALING_ACTIVE 1" in superResolutionSource)
check("#define uval_taaJitter SRJitterOffset" in superResolutionSource)
check(Regex("#if UPSCALER_RECONSTRUCTION_ACTIVE").findAll(renderScaleSource).count() == 2)
check("\"supports_frame_generation_only\": true" in superResolutionConfigSource)
check("\"hdr\": true" in superResolutionConfigSource)
check("\"motion_jittered\": false" in superResolutionConfigSource)
check("\"type\": \"BEFORE\"" in superResolutionConfigSource)
check("\"pass\": \"composite54\"" in superResolutionConfigSource)
check("\"src\": \"autotex31\"" in superResolutionConfigSource)
check("\"src\": \"autotex30\"" in superResolutionConfigSource)
check("\"region\": [0, 0, 1, 1]" in superResolutionConfigSource)
check("\"auto_exposure\"" !in superResolutionConfigSource)
check("size.buffer.colortex30=1 1" in propertiesSource)
check("size.buffer.colortex31=RENDER_SCALE_VALUE RENDER_SCALE_VALUE" in propertiesSource)
check("#if SR_ENABLE && SR_SHOULD_APPLY_SCALE" in propertiesSource)
check("image.uimg_superResolutionBloom=usam_superResolutionBloom RGBA RGBA16F HALF_FLOAT false true 1.0 1.0" in propertiesSource)
check("uniform sampler2D usam_superResolutionBloom;" in texturesSource)
check("#elif EXTERNAL_SR_UPSCALING_ACTIVE" in bloomSource)
check("uimg_superResolutionBloom" in bloomSource && "upscaledBloom_mipTile" in bloomSource)
check("const int colortex30Format = R32F" in configsSource)
check("const bool colortex30Clear = false" in configsSource)
check("const int colortex31Format = RG16F" in configsSource)
check("imageStore(uimg_superResolutionExposure, texelPos, vec4(exp2(global_aeData.expValues.z)))" in motionSource)
val solidProcessData2Source = solidGbufferSource.substringAfter("void processData2()").substringBefore("void processData1()")
check("defined(GBUFFER_PASS_ENTITY) || defined(GBUFFER_PASS_PARTICLE) || defined(GBUFFER_PASS_HAND)" in solidProcessData2Source)
check("GBUFFER_PASS_DYNAMIC" !in solidProcessData2Source)
