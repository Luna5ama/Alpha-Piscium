...
//    }
    ...
    if (inViewPort(ivec4(0, 512, 512, 512), debugTexCoord)) {
        outputColor.rgb = gammaCorrect(persistent_rtwsm_importance2D_sample(debugTexCoord).r * 0.5).rrr;
    }
    if (inViewPort(ivec4(0, 1024 + 4, 512, 16), debugTexCoord)) {
        outputColor.rgb = gammaCorrect(persistent_rtwsm_importance1D_sample(vec2(debugTexCoord.x, IMAP1D_X_V)).r * 0.1).rrr;
    }
    if (inViewPort(ivec4(512 + 4, 512, 16, 512), debugTexCoord)) {
        outputColor.rgb = gammaCorrect(persistent_rtwsm_importance1D_sample(vec2(debugTexCoord.y, IMAP1D_Y_V)).r * 0.1).rrr;
    }
    if (inViewPort(ivec4(0, 1024 + 4 + 16 + 4, 512, 16), debugTexCoord)) {
        float v = persistent_rtwsm_warp_sample(vec2(debugTexCoord.x, WARP_X_V)).r;
        outputColor.rgb = vec3(max(v, 0.0), max(-v, 0.0), 0.0);
    }
    if (inViewPort(ivec4(0, 1024 + 4 + 16 + 4 + 16 + 4, 512, 16), debugTexCoord)) {
        vec4 warpTexelSize = persistent_rtwsm_warpTexelSize_sample(debugTexCoord);
        float v = warpTexelSize.r * 2.0 - 1.0;
        outputColor.rgb = vec3(max(v, 0.0), max(-v, 0.0), 0.0);
    }
    if (inViewPort(ivec4(512 + 4 + 16 + 4, 512, 16, 512), debugTexCoord)) {
        float v = persistent_rtwsm_warp_sample(vec2(debugTexCoord.y, WARP_Y_V)).r;
        outputColor.rgb = vec3(max(v, 0.0), max(-v, 0.0), 0.0);
    }
    if (inViewPort(ivec4(512 + 4 + 16 + 4 + 16 + 4, 512, 16, 512), debugTexCoord)) {
        vec4 warpTexelSize = persistent_rtwsm_warpTexelSize_sample(debugTexCoord);
        float v = warpTexelSize.g * 2.0 - 1.0;
        outputColor.rgb = vec3(max(v, 0.0), max(-v, 0.0), 0.0);
    }
    #endif
