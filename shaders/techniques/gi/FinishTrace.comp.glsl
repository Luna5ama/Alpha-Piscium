...
void main() {
    ...
    uint indirectData = indirectComputeData[gl_WorkGroupID.x];

    uint binIdx = bitfieldExtract(indirectData, 2, 30);
    uint localGroupIdx = bitfieldExtract(indirectData, 0, 2);

    uint threadIdx = gl_SubgroupID * gl_SubgroupSize + gl_SubgroupInvocationID;

    uint binLocalIndex = localGroupIdx * 256u + threadIdx;
