...
const vec2 workGroupsRender = vec2(POST_PROCESS_SCALE_HALF, POST_PROCESS_SCALE_HALF);
...
vec4 bloom_readInputUp(ivec2 coord) {
    vec2 readPosUV = vec2((vec2(coord) + 0.5) * 0.5 + inputStartTexel) * POST_PROCESS_IMAGE_SIZE_RCP;
    readPosUV = clamp(readPosUV, inputMinUV, inputMaxUV);
