...
vec2 textile_texelToUV(vec2 texelPos, vec2 tileOffset, vec2 tileSize, vec2 textureSizeRcp) {
    vec2 textureTexelPos = clamp(texelPos, vec2(0), tileSize - 1.0) + tileOffset;
