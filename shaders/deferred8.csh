...
vec2 sceneToScreen(vec3 scenePos) {
    vec4 clipPos = shared_camProj * (gbufferModelView * vec4(scenePos, 1.0));
...
void main() {
    ...
    ivec2 texelPos4x = ivec2(gl_GlobalInvocationID.xy);
    vec2 jitter = rand_stbnVec2(texelPos4x, frameCounter);
    vec2 screenPos1x = (vec2(texelPos4x) + jitter) / 1.0 / SHADOW_MAP_SIZE.x;
    vec4 waterNormalAndMast = texture(usam_shadow_waterNormal, screenPos1x);
    barrier();

    if (waterNormalAndMast.a >= 0.9999) {
        vec2 screenPos = texture(usam_shadow_unwarpedUV, screenPos1x).rg;
        float shadowDepth = texture(shadowtex0, screenPos1x).r;
        ...
        const float RIOR = 1.00029 / 1.3333;
...
//            maxT = rayDirScreen.y != 0.0f ? min((float(rayDirScreen.y > 0.0f) - pRayStart.y) * rcpRayDirScreen.y, maxT) : maxT;
            ...
            if (intersects) {
                ...
                if (pRayStart.z > startDepth) {
                    ...
                    float jitterStep = rand_stbnVec1(texelPos4x, frameCounter);
                    ...
                    if (currT > 0.0 && currT < 1.0) {
                        ...
                        if (all(greaterThanEqual(camTexelPos, ivec2(0))) && all(lessThan(camTexelPos, uval_mainImageSizeI))) {
                            ...
                            float area = texture(usam_shadow_pixelArea, screenPos1x).r / camAreaSize;
