...
void main() {
    ...
    if (all(lessThan(texelPos, uval_mainImageSizeI))) {
        ...
        vec4 finalColor = texelFetch(usam_overlays, texelPos, 0);
        ...
        for (uint i = 0u; i < MAX_ITER; ++i) {
            ...
            float fi = float(i) / float(count - 1u);

            vec4 data1 = ssbo_testBuffer[i];
            vec4 data2 = ssbo_testBuffer[i + 2048u];

            vec2 screenPos = data1.xy;
            vec2 texelPos = vec2(screenPos * uval_mainImageSize);

            float distanceToPoint = distance(texelPos, texelPosCenter);

            float pointAlpha = smoothstep(4.0, 2.0, distanceToPoint) ;

            int cellLevel = int(data2.x) + 1;
            vec2 cellIdx = data1.zw;

            vec2 cellStartPoint = ldexp(cellIdx, ivec2(cellLevel));
            vec2 cellEndPoint = ldexp(cellIdx + vec2(1.0), ivec2(cellLevel));
            vec2 cellCenterPoint = ldexp(cellIdx + vec2(0.5), ivec2(cellLevel));
            float cellSize = ldexp(1.0, cellLevel);
            vec2 distanceXY = abs(texelPosCenter - cellCenterPoint);
            float manhattanDistToCellCenter = max(distanceXY.x, distanceXY.y);

            float lineRange = 2.0;

            float boxAlpha = 0.0;
//            boxAlpha = smoothstep(cellSize - lineRange, cellSize, manhattanDistToCellCenter - cellSize);
//            boxAlpha *= smoothstep(cellSize + lineRange, cellSize, manhattanDistToCellCenter - cellSize);

//            boxAlpha = smoothstep(lineEnd, lineStart, distance(texelPosCenter, cellStartPoint));
//            boxAlpha += smoothstep(lineEnd, lineStart, distance(texelPosCenter, cellEndPoint));
//            boxAlpha *= smoothstep(lineEnd, lineStart, -(texelPosCenter.x - cellStartPoint.x));
//            boxAlpha *= smoothstep(lineEnd, lineStart, (texelPosCenter.x - cellEndPoint.x));
//            boxAlpha *= smoothstep(lineEnd, lineStart, -(texelPosCenter.y - cellStartPoint.y));
//            boxAlpha *= smoothstep(lineEnd, lineStart, (texelPosCenter.y - cellEndPoint.y));

            vec3 stepColor = interpolateTurbo(fi);
            float stepAlpha = saturate(max(pointAlpha, boxAlpha));
            finalColor.rgb = mix(finalColor.rgb, stepColor, stepAlpha);
            finalColor.a = saturate(finalColor.a + stepAlpha);
        }

        imageStore(uimg_overlays, texelPos, finalColor);
    }
}
