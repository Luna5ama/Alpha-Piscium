...
vec3 bsdf_diffuseHammon(Material material, float NDotL, float NDotV, float NDotH, float LDotV) {
    ...
    if (NDotL > 0.0) {
        ...
        float fresnelNL = fresnel_dielectricDielectric_transmittance(max(NDotL, 1e-2), AIR_IOR, ior_f0ToIor(material.f0));
        float fresnelNV = fresnel_dielectricDielectric_transmittance(max(NDotV, 1e-2), AIR_IOR, ior_f0ToIor(material.f0));
