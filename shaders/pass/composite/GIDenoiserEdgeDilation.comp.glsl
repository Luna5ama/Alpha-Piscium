...
void main() {
    ...
    if (all(lessThan(texelPos, uval_mainImageSizeI))) {
        ...
        float edgeMaskSum = shared_edgeMask[localPos.y - 1][localPos.x - 1];
        edgeMaskSum += shared_edgeMask[localPos.y - 1][localPos.x];
        edgeMaskSum += shared_edgeMask[localPos.y - 1][localPos.x + 1];
        edgeMaskSum += shared_edgeMask[localPos.y][localPos.x - 1];
        edgeMaskSum += shared_edgeMask[localPos.y][localPos.x];
        edgeMaskSum += shared_edgeMask[localPos.y][localPos.x + 1];
        edgeMaskSum += shared_edgeMask[localPos.y + 1][localPos.x - 1];
        edgeMaskSum += shared_edgeMask[localPos.y + 1][localPos.x];
        edgeMaskSum += shared_edgeMask[localPos.y + 1][localPos.x + 1];
