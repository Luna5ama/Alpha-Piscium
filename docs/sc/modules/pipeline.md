...
| [`RenderVolumetric`](../../../shaders/techniques/atmospherics/clouds/RenderVolumetric.comp.glsl)、`clouds/ss/Accum`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 可选 cumulus render → temporal/spatial accumulation                                                                      |
...
1. 初始 ray sort/finish（`SETTING_GI_INITIAL_SST_STEPS >= 64` 时启用）。
2. temporal reuse；可选 duplication-map decorrelation。
3. 最多四个 paired spatial-reuse 批次；阈值对应 reuse count `> 0/7/14/21`，由 SSBO 0 offset 48 indirect dispatch。
4. paired spatial shade → spatial ray sort → trace。
5. denoiser accumulation → optional anti-firefly → GI mip → history fix → optional blur/post-blur。
6. 可选 SST-step debug。
