...
dispatch。`history_restir_reservoirTemporal1/2`、`history_restir_prevSample`、`history_restir_prevHitNormal` 保存 temporal
输入，`transient_restir_spatialInput` 和 `transient_restir_pairwiseMISMetadata` 连接 spatial 阶段。所有 tile 定义见 [
